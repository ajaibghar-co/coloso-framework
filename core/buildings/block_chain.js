...
export function main(coord, context, cursor, buffer) {
  ...
  let mod1 =
    Math.floor(
      Math.abs(
        (coord.x / context.rows) * 10.0 +
          sin(st.y * 2.0) * 2.0 * sign +
          t * 2.0 * sign
      )
    ) % density.length;
  ...
  let mod2 = Math.floor(
    Math.abs(
      sin(cf * 2.0) * 2.0 * sin(st.y * 10.0 + st.x * 10.0 + t) * colors.length
    )
  );
  mod2 = Math.floor(
    Math.abs(sin(st.y * 10.0 + st.x * 10.0 + t)) +
      length(addN(vec2(cf, cf), -t * 0.1)) * colors.length * 4.0
  );
